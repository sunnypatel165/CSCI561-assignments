...
def run_minimax(grid, r_points, b_points, player):
    ...
    if player == 'r':
        if len(r_reward_list) == 0:    # no available white space
            ...
            local_r_best = -float("inf")
            local_b_best = -float("inf")
            ...
            for e_space in r_reward_list:
                ...
                returned_r_best, returned_b_best = run_minimax(temp_grid, r_points, b_points, 'b')
                ...
                if local_r_best < returned_r_best:
                    local_r_best = returned_r_best
                    local_b_best = returned_b_best

            return local_r_best, local_b_best
        ...
        if len(b_reward_list) == 0:     # no available white space
            ...
            for e_space in b_reward_list:
                ...
                # recursively call minimax again
                returned_r_best, returned_b_best = run_minimax(temp_grid, r_points, b_points, 'r')
                ...
                # capture the best values and update local maximizing variables
                if local_b_best < returned_b_best:
                    local_r_best = returned_r_best
                    local_b_best = returned_b_best
