...
def calculate_score(grid, grid_size):
    ...
    for i in xrange(grid_size):
        for j in xrange(grid_size):
            if grid[i][j] == State.P1_AREA or grid[i][j] == State.P1_LASER or grid[i][j] == State.BOTH_AREA:
                State.P1_score = State.P1_score + 1
            elif grid[i][j] == State.P2_AREA or grid[i][j] == State.P2_LASER or grid[i][j] == State.BOTH_AREA:
                p2_score = p2_score + 1
    return State.P1_score, p2_score
...
def get_empty_slots(grid, grid_size, player):
    dprint("getting State.EMPTY slots on ")
...
def min_value(grid, grid_size, player, alpha, beta):
    ...
    if player == State.AI:
        ...
        next_player = State.AI
    if check_game_over(grid, grid_size):
        State.P1_score, p2_score = calculate_score(grid, grid_size)
        if State.P1_score > p2_score:
            return [10, -1, -1, -1, -1]
        else:
            return [-10, -1, -1, -1, -1]
...
def minimax(grid, grid_size, player):
    return max_value(grid, grid_size, State.P1, -1000, 1000)
    # available_moves = get_empty_slots(grid, grid_size, player)
    # # print "avState.AIable moves " + str(available_moves)
    # score = [-float('inf'), -1, -1, -1, -1]
    #
    # for i in xrange(len(available_moves)):
    #     # state = [row[:] for row in grid]
    ...
    #     mark_player(state, grid_size, available_moves[i][0], available_moves[i][1], player - 6)
    #     # print_grid(state, grid_size)
    #     new_score = min_value(state, grid_size, State.AI, -float('inf'), float('inf'))
